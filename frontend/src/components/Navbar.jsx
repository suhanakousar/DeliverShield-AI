...
import { HiShieldCheck, HiMenu, HiX, HiHome, HiClipboardList, HiCash, HiCog, HiLogout, HiUserAdd } from 'react-icons/hi';

const Navbar = () => {
  const [mobileOpen, setMobileOpen] = useState(false);
  const { currentWorker, isAdmin, logout } = useApp();
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-slate-700/50 bg-slate-800/90 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2 group" onClick={closeMobile}>
            <div className="w-9 h-9 bg-gradient-to-br from-primary-500 to-primary-700 rounded-lg flex items-center justify-center shadow-lg shadow-primary-500/25 group-hover:shadow-primary-500/40 transition-shadow">
            ...
            <span className="flex items-center gap-1 ml-2 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/30">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {workerId && (
              <button
                onClick={logout}
                className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-colors duration-200"
              >
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden border-t border-slate-700/50 bg-slate-800/95 backdrop-blur-xl animate-slide-down">
          <div className="px-4 py-3 space-y-1">
            ...
            <Link to="/admin/claims" className={navLinkClasses('/admin/claims')} onClick={closeMobile}>
              <HiClipboardList className="w-4 h-4" />
              Admin Claims
            </Link>
            <Link to="/admin/simulate" className={navLinkClasses('/admin/simulate')} onClick={closeMobile}>
              <HiCash className="w-4 h-4" />
              Simulate Disruption
            </Link>
            ...
            {workerId && (
              <>
                ...
                <button
                  ...
                  className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm font-medium text-red-400 hover:bg-red-500/10 transition-colors duration-200"
                >
                  <HiLogout className="w-4 h-4" />
                  Logout
                </button>
